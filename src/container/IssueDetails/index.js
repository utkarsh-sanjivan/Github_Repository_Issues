...
import { Skeleton, Avatar, Empty } from 'antd';
...
class IssueDetails extends React.PureComponent {
  constructor (props) {
    ...
    componentDidUpdate(prevProps) {
        ...
        if (
            !prevProps.issueList.issueError && this.props.issueList.issueError
        ) {
            this.setState({ issueNotFound: true })
        }
    }
    ...
    render () {
        ...
        this.props.issueList.issueLoading?
            <Skeleton 
                ...
                active
            />
            : this.state.issueNotFound?
                <Empty 
                    className="table-empty-image"
                    description={'Issues Unavailable'}
                />
